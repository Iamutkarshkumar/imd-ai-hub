...
export default function WeatherDashboard() {
  ...
  // ── Hourly forecast — always Open-Meteo (IMD has no hourly data) ──────────
  useEffect(()=>{
    if(!selected?.lat||!selected?.lon) return;
    let active = true;
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${selected.lat}&longitude=${selected.lon}&hourly=temperature_2m,precipitation_probability,weather_code&timezone=auto`;
    fetch(url).then(r=>r.json()).then(data=>{
      ...
      for(let i=0;i<9;i++){
        ...
        newHourly.push({
          ...
          icon: getWeatherIcon(wmo.n, isHourDay, 24),
        });
      }
  ...
  // ── Weekly forecast — read forecast_json from IMD database record ─────────
  useEffect(()=>{
    if(!selected) return;

    // Parse the forecast_json stored by the backend updater
    // Falls back gracefully if the field is missing or malformed
    try {
      const raw = selected.forecast_json;
      if(!raw) { setExtended(prev=>({...prev,weekly:[]})); return; }
      ...
      const weekly = days
        ...
        .map(d => {
          // Map IMD forecast text → weather icon
          // If text is null (days 6-7 from Open-Meteo), use a generic clear icon
          const condText = d.text || 'Clear Sky';
          return {
            day:       d.day,
            high:      Math.round(d.high ?? 0),
            low:       Math.round(d.low  ?? 0),
            condition: condText,
            icon:      getWeatherIcon(condText, true, 26),
          };
        });

      setExtended(prev=>({...prev, weekly}));
    } catch(err) {
      console.error("forecast_json parse error:", err);
      setExtended(prev=>({...prev, weekly:[]}));
    }
  },[selected?.city, selected?.forecast_json]);
        ...
        <div style={{background:'linear-gradient(90deg,#7f1d1d,#991b1b)',height:34,display:'flex',alignItems:'center',overflow:'hidden',flexShrink:0,zIndex:20}}>
          ...
          <div style={{flex:1,overflow:'hidden'}}>
            <div style={{
              ...
              whiteSpace:'nowrap',
              animation:`ticker ${Math.max(20, activeAlerts.length * 18)}s linear infinite`,
              fontSize:11,
              color:'#fecaca',
