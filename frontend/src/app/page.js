...
//    Switched from decorative display fonts to a clean, professional pairing:
//    Inter (headings + UI) and Source Sans 3 (body). Both are built for
//    high legibility and read as a credible, institutional dashboard
//    rather than a stylised consumer app.                                    */
// const FONT_LINK = "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=Source+Sans+3:ital,opsz,wght@0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700;1,9..40,400&family=JetBrains+Mono:wght@400;500;600&display=swap";
...
// .city-row{transition:background 0.18s,transform 0.18s;cursor:pointer;}
// .city-row:hover{background:rgba(255,255,255,0.08)!important;transform:translateX(3px);}
// .tab{transition:all 0.2s ease;cursor:pointer;border:none;font-family:${FONT_BODY};}
// .tab:hover{opacity:1!important;}
// .chip{transition:all 0.15s ease;cursor:pointer;}
// .chip:hover{opacity:1!important;transform:translateY(-1px);}
// .icon-btn{transition:all 0.2s ease;cursor:pointer;}
...
//   return (
//     <div style={{textAlign:'right'}}>
//       <div style={{fontFamily:FONT_MONO,fontSize:23,fontWeight:600,color:'#fff',letterSpacing:'0.04em',lineHeight:1}}>
//         {hh}<span style={{color:accent,animation:'blink 2s step-start infinite'}}>:</span>{mm}
//         <span style={{fontSize:15,color:'rgba(255,255,255,0.45)',marginLeft:4}}>
//           {ss}{format==='12h'&&<span style={{fontSize:12,marginLeft:2}}>{ampm}</span>}
//         </span>
//       </div>
//       <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginTop:3,fontWeight:500}}>{date}</div>
//     </div>
//   );
// }
...
//         <circle cx={sx} cy={cy} r="3"  fill={accent} opacity="0.5"/>
//         <circle cx={ex} cy={ey} r="3"  fill={accent} opacity="0.5"/>
//         <text x={sx-4} y={cy+16} textAnchor="middle" fontSize="11" fontWeight="600" fill="rgba(255,255,255,0.5)">{leftLabel}</text>
//         <text x={ex+4} y={cy+16} textAnchor="middle" fontSize="11" fontWeight="600" fill="rgba(255,255,255,0.5)">{rightLabel}</text>
//       </svg>
//       <div style={{fontSize:13,color:'rgba(255,255,255,0.45)',marginTop:-4,fontWeight:500}}>{timeText}</div>
//     </div>
//   );
// }
...
//         <circle cx="50" cy="50" r="30" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" strokeDasharray="3 4"/>
//         {[...Array(36)].map((_,i)=>{ const a=i*10*Math.PI/180,r1=i%9===0?38:i%3===0?41:43; return <line key={i} x1={50+r1*Math.sin(a)} y1={50-r1*Math.cos(a)} x2={50+45*Math.sin(a)} y2={50-45*Math.cos(a)} stroke="rgba(255,255,255,0.12)" strokeWidth={i%9===0?1.2:0.6}/>; })}
//         {dirs.map((d,i)=>{ const a=i*45*Math.PI/180; return <text key={d} x={50+40*Math.sin(a)} y={50-40*Math.cos(a)} textAnchor="middle" dominantBaseline="middle" fontSize={d.length===1?'9':'7'} fontWeight={d==='N'?800:600} fill={d==='N'?accent:'rgba(255,255,255,0.45)'} fontFamily={FONT_MONO}>{d}</text>; })}
//         <defs><marker id="arrowhead" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto"><polygon points="0 0,6 3,0 6" fill={accent}/></marker></defs>
//         <line x1={tX} y1={tY} x2={nX*0.96+(50*0.04)} y2={nY*0.96+(50*0.04)} stroke={accent} strokeWidth="2.5" strokeLinecap="round" markerEnd="url(#arrowhead)" opacity="0.9"/>
//         <circle cx="50" cy="50" r="4" fill={accent} opacity="0.4"/>
//         <circle cx="50" cy="50" r="2.5" fill={accent}/>
//       </svg>
//       <div style={{textAlign:'center'}}>
//         <div style={{fontSize:30,fontWeight:800,color:'#fff',fontFamily:FONT_HEAD,letterSpacing:'-0.02em'}}>{speed}<span style={{fontSize:15,color:'rgba(255,255,255,0.45)',fontWeight:500}}> km/h</span></div>
//         <div style={{fontSize:13,color:accent,marginTop:2,fontWeight:600}}>{compassDir(deg)} · Gusts {gust} km/h</div>
//       </div>
//     </div>
//   );
// }
...
//               background:'rgba(255,255,255,0.04)',borderRadius:12,padding:'10px 8px',
//               border:`1px solid rgba(255,255,255,${i===0?0.15:0.06})`}}>
//               <div style={{fontSize:12,color:'rgba(255,255,255,0.5)',fontFamily:FONT_MONO,fontWeight:600}}>{h.hour}</div>
//               <div style={{fontSize:28,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center'}}>
//                 {h.icon}
//               </div>
//               <div style={{height:44,width:4,background:'rgba(255,255,255,0.08)',borderRadius:4,position:'relative'}}>
//                 <div style={{position:'absolute',bottom:0,width:'100%',borderRadius:4,height:`${pct}%`,background:`linear-gradient(to top,${accent},${accent}88)`}}/>
//               </div>
//               <div style={{fontSize:15,fontWeight:700,color:'#fff'}}>{h.temp}°</div>
//               {h.pop!=null&&<div style={{fontSize:11,color:'#38bdf8',fontFamily:FONT_MONO,fontWeight:600}}>{h.pop}%</div>}
//             </div>
//           );
//         })}
//       </div>
...
//             border:`1px solid ${i===0?`${accent}30`:'rgba(255,255,255,0.05)'}`,
//             animation:`slideRight 0.3s ${i*0.06}s both`}}>
//             <div style={{fontSize:15,fontWeight:600,color:'rgba(255,255,255,0.7)',width:38,fontFamily:FONT_BODY}}>{d.day}</div>
//             <div style={{fontSize:26,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center',width:32}}>
//               {d.icon}
//             </div>
//             <div style={{fontSize:13,fontWeight:500,color:'rgba(255,255,255,0.45)',width:110,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.condition}</div>
//             <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}>
//               <span style={{fontSize:14,fontWeight:600,color:'rgba(255,255,255,0.45)',minWidth:32,textAlign:'right'}}>{d.low}°</span>
//               <div style={{flex:1,height:4,background:'rgba(255,255,255,0.07)',borderRadius:4,position:'relative'}}>
//                 <div style={{position:'absolute',left:`${lo}%`,width:`${hi-lo}%`,height:'100%',background:`linear-gradient(to right,#38bdf8,${accent})`,borderRadius:4}}/>
//               </div>
//               <span style={{fontSize:14,color:'#fff',fontWeight:700,minWidth:32}}>{d.high}°</span>
//             </div>
//           </div>
//         );
//       })}
...
//   return (
//     <div style={{position:'relative',flex:1,maxWidth:440}}>
//       <div style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',color:'rgba(255,255,255,0.4)',pointerEvents:'none',fontSize:15}}>
//         <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
//       </div>
//       <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||"Search city…"} className="search-input"
//         style={{width:'100%',background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',
//           borderRadius:12,padding:'10px 36px 10px 38px',color:'#fff',fontSize:14,fontWeight:500,
//           fontFamily:FONT_BODY,transition:'all 0.25s',backdropFilter:'blur(10px)'}}
//         onFocus={e=>{e.target.style.border=`1px solid ${accent}66`;e.target.style.background='rgba(255,255,255,0.1)';}}
//         onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.12)';e.target.style.background='rgba(255,255,255,0.07)';}}/>
//       {value&&<button onClick={onClear} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',
//         background:'rgba(255,255,255,0.12)',border:'none',borderRadius:'50%',width:18,height:18,
...
//       padding:'16px 18px',border:'1px solid rgba(255,255,255,0.08)',overflow:'hidden',position:'relative'}}>
//       <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
//         <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_BODY}}>{label}</div>
//         <span style={{fontSize:24,lineHeight:1,display:'flex',alignItems:'center'}}>{icon}</span>
//       </div>
//       <div style={{fontSize:28,fontWeight:800,color:'#fff',fontFamily:FONT_HEAD,lineHeight:1,letterSpacing:'-0.01em'}}>{value}</div>
//       {sub&&<div style={{fontSize:13,fontWeight:500,color:subColor||'rgba(255,255,255,0.5)',marginTop:5}}>{sub}</div>}
//       {progress!=null&&(
//         <div style={{marginTop:10,height:3,background:'rgba(255,255,255,0.07)',borderRadius:4}}>
//           <div style={{height:'100%',width:`${Math.min(100,progress)}%`,borderRadius:4,background:accentColor||'rgba(255,255,255,0.4)',transition:'width 0.5s ease'}}/>
//         </div>
...
//   if (!active||!payload?.length) return null;
//   return (
//     <div style={{background:'rgba(10,20,40,0.95)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,padding:'9px 13px',fontSize:13,backdropFilter:'blur(16px)'}}>
//       <div style={{color:'rgba(255,255,255,0.55)',marginBottom:4,fontWeight:600}}>{label}</div>
//       {payload.map((p,i)=>(
//         <div key={i} style={{color:p.color||'#fff',display:'flex',gap:8,alignItems:'center'}}>
//           <span style={{width:8,height:8,borderRadius:'50%',background:p.color||'#fff',flexShrink:0}}/>
//           <span style={{color:'rgba(255,255,255,0.65)',fontWeight:500}}>{p.name}:</span>
...
//   const HBar=({data,title,barColor})=>(
//     <div style={{flex:1}}>
//       <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:12,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>{title}</div>
//       <div style={{height:data.length*28}}>
//         <ResponsiveContainer width="100%" height="100%">
//           <BarChart data={data} layout="vertical" margin={{top:0,right:36,left:4,bottom:0}}>
//             <XAxis type="number" domain={['auto','auto']} hide/>
//             <YAxis type="category" dataKey="city" width={78} tick={{fontSize:13,fontWeight:600,fill:'rgba(255,255,255,0.65)',fontFamily:FONT_BODY}} axisLine={false} tickLine={false}/>
//             <Tooltip content={<ChartTooltip/>} cursor={{fill:'rgba(255,255,255,0.04)'}}/>
//             <Bar dataKey="temp" name="Temp °C" fill={barColor} radius={[0,5,5,0]} barSize={14}
//               label={{position:'right',fontSize:12,fontWeight:700,fill:'rgba(255,255,255,0.6)',formatter:v=>`${v}°`}}/>
//           </BarChart>
//         </ResponsiveContainer>
//       </div>
//     </div>
...
//   return (
//     <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:16}}>
//       <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:16,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO,display:'flex',alignItems:'center',gap:8}}>
//         <WiHot size={18} color="#ef4444"/> Hottest &amp; <WiSnow size={18} color="#38bdf8"/> Coldest Cities
//       </div>
//       <div style={{display:'flex',gap:24}}>
//         <HBar data={hottest} title="Top 10 Hottest" barColor="#ef4444"/>
...
//         background:isUser?accent:'rgba(255,255,255,0.07)',color:isUser?'#000':'rgba(255,255,255,0.92)',
//         fontSize:14,fontWeight:500,lineHeight:1.55,border:isUser?'none':'1px solid rgba(255,255,255,0.08)',
...
//     setQuery(''); setIsTyping(true);
//     try {
//       const res=await fetch(`${API_BASE}/chat`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:m})});
//       if(!res.ok) throw new Error(`${res.status}`);
//       const data=await res.json();
//       let reply=typeof data.ai_response==='string'?data.ai_response:'Sorry, could not process that.';
//       reply=reply.replace(/(?:\s*End\.\s*)+$/gi,'').trim();
...
//         </div>
//         <div>
//           <div style={{fontWeight:700,fontSize:15,fontFamily:FONT_HEAD}}>AI Meteorologist</div>
//           <div style={{fontSize:12,fontWeight:500,color:'rgba(255,255,255,0.4)',marginTop:1}}>
//             {isListening?<span style={{color:'#fca5a5'}}>● Listening…</span>:'Powered by Llama 3.1'}
//           </div>
//         </div>
//         <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8}}>
//           {isListening&&(
//             <div style={{display:'flex',alignItems:'center',gap:5,background:'rgba(239,68,68,0.15)',border:'1px solid rgba(239,68,68,0.3)',borderRadius:20,padding:'3px 10px',fontSize:11,fontWeight:700,color:'#fca5a5'}}>
//               <span style={{width:6,height:6,borderRadius:'50%',background:'#ef4444',animation:'blink 1s ease infinite'}}/>REC
//             </div>
//           )}
//           <div style={{width:7,height:7,borderRadius:'50%',background:'#4ade80',animation:'blink 2.5s ease infinite'}}/>
...
//         {quickQs.map(q=>(
//           <button key={q} className="chip" onClick={()=>setQuery(q)}
//             style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.7)',borderRadius:20,padding:'5px 11px',fontSize:11,fontWeight:600,cursor:'pointer',fontFamily:FONT_BODY,opacity:0.9}}>
//             {q}
//           </button>
//         ))}
//       </div>
...
//                 ))}
//               </div>
//               <span style={{fontSize:13,fontWeight:600,color:accent,fontStyle:'italic'}}>{query||'Listening…'}</span>
//             </div>
//           </div>
//         )}
//         {isTyping&&(
...
//       </div>
//       {voiceError&&(
//         <div style={{padding:'6px 14px',fontSize:12,fontWeight:600,color:'#fca5a5',background:'rgba(220,38,38,0.1)',borderTop:'1px solid rgba(220,38,38,0.2)',flexShrink:0}}>⚠ {voiceError}</div>
//       )}
//       <div style={{padding:'12px 14px',borderTop:'1px solid rgba(255,255,255,0.07)',flexShrink:0}}>
//         <div style={{display:'flex',gap:8,background:'rgba(255,255,255,0.06)',borderRadius:13,padding:'4px 4px 4px 14px',border:`1px solid ${isListening?`${accent}60`:'rgba(255,255,255,0.1)'}`,transition:'border-color 0.2s',backdropFilter:'blur(10px)'}}>
//           <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={handleKey} disabled={isTyping}
//             style={{flex:1,background:'transparent',border:'none',outline:'none',color:'#fff',fontSize:14,fontWeight:500,fontFamily:FONT_BODY,opacity:isTyping?0.5:1}}
//             placeholder={isListening?'Listening — speak now…':isTyping?'Thinking…':'Ask anything about weather…'}/>
//           {voiceSupported&&(
//             <button onClick={toggleVoice} disabled={isTyping} title={isListening?'Stop':'Voice input'}
//               style={{background:isListening?'rgba(239,68,68,0.25)':'rgba(255,255,255,0.08)',border:isListening?'1px solid rgba(239,68,68,0.5)':'1px solid rgba(255,255,255,0.12)',borderRadius:8,padding:'7px 10px',cursor:isTyping?'not-allowed':'pointer',fontSize:14,opacity:isTyping?0.4:1,transition:'background 0.2s,border 0.2s',display:'flex',alignItems:'center',justifyContent:'center'}}>
...
//           )}
//           <button onClick={handleChat} disabled={isTyping||!query.trim()}
//             style={{background:query.trim()?accent:'rgba(255,255,255,0.1)',border:'none',borderRadius:10,padding:'8px 16px',color:query.trim()?'#000':'rgba(255,255,255,0.3)',fontWeight:800,fontSize:14,cursor:(isTyping||!query.trim())?'not-allowed':'pointer',fontFamily:FONT_HEAD,opacity:isTyping?0.45:1,transition:'all 0.2s',letterSpacing:'-0.01em'}}>
//             ↑
//           </button>
//         </div>
//         {voiceSupported&&!isListening&&<div style={{fontSize:12,fontWeight:500,color:'rgba(255,255,255,0.3)',marginTop:5,textAlign:'center'}}>🎤 tap mic to speak · Enter to send</div>}
//         {!voiceSupported&&<div style={{fontSize:12,fontWeight:500,color:'rgba(255,255,255,0.22)',marginTop:5,textAlign:'center'}}>Voice input not supported in this browser</div>}
//       </div>
//     </div>
//   );
// }
...
//   /* ── Hourly timeline: sourced from the selected city's own record
//        (hourly_timeline JSON column, populated by imd_live_updater.py).
//        No more direct browser → Open-Meteo calls.                       */
//   useEffect(()=>{
//     if(!selected) return;

//     // Hourly: pull straight from the record already in `stats`/`selected`
//     if (Array.isArray(selected.hourly_timeline) && selected.hourly_timeline.length) {
//       const newHourly = selected.hourly_timeline.slice(0, 18).map((h, i) => {
//         const hh = parseInt((h.time || '00:00').split(':')[0], 10);
//         const isHourDay = hh >= 6 && hh < 18;
//         let h12 = hh % 12 || 12;
//         const ampm = hh >= 12 ? 'PM' : 'AM';
//         return {
//           hour: i === 0 ? 'Now' : `${h12}${ampm}`,
//           temp: Math.round(h.temp),
//           pop: null,
//           icon: getWeatherIcon(h.condition || '', isHourDay, 24),
//         };
//       });
//       setExtended(prev => ({ ...prev, hourly: newHourly }));
//     } else {
//       setExtended(prev => ({ ...prev, hourly: [] }));
//     }
//   }, [selected?.city, selected?.hourly_timeline]);

//   /* ── Weekly forecast: now fetched from our own backend's
//        /forecast/{city} endpoint, which reads the weather_forecasts
//        table (the hybrid IMD + Open-Meteo 7-day data).                  */
//   useEffect(()=>{
//     if(!selected?.city) return;
//     let active = true;
//     fetch(`${API_BASE}/api/weather/${encodeURIComponent(selected.city)}`)
//       .then(r => { if(!r.ok) throw new Error('no forecast'); return r.json(); })
//       .then(rows => {
//         if(!active) return;
//         const daysArr=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
//         const newWeekly = rows.map((d, i) => {
//           const dateObj = d.date ? new Date(d.date) : null;
//           const dayLabel = i === 0 ? 'Today' : (dateObj ? daysArr[dateObj.getDay()] : `Day ${d.day_number}`);
//           return {
//             day: dayLabel,
//             high: Math.round(d.max_temp ?? 0),
//             low: Math.round(d.min_temp ?? 0),
//             condition: d.condition || 'N/A',
//             icon: getWeatherIcon(d.condition || '', true, 26),
//           };
//         });
//         setExtended(prev => ({ ...prev, weekly: newWeekly }));
//       })
//       .catch(() => { if(active) setExtended(prev => ({ ...prev, weekly: [] })); });
//     return () => { active = false; };
//   }, [selected?.city]);

//   useEffect(()=>{
//     fetch(`${API_BASE}/weather-stats`)
//       .then(r=>r.json())
//       .then(data=>{ const e=data.map(enrichCity); setStats(e); setSelected(e[0]||null); })
//       .catch(()=>{ const d=generateDemoData(); setStats(d); setSelected(d[0]); });
//   },[]);
...
//       {/* ALERT TICKER */}
//       {activeAlerts.length>0&&(
//         <div style={{background:'linear-gradient(90deg,#7f1d1d,#991b1b)',height:36,display:'flex',alignItems:'center',overflow:'hidden',flexShrink:0,zIndex:20}}>
//           <div style={{background:'rgba(0,0,0,0.3)',height:'100%',padding:'0 14px',display:'flex',alignItems:'center',fontSize:12,fontWeight:800,letterSpacing:'0.1em',flexShrink:0,gap:6}}>
//             <span style={{width:6,height:6,borderRadius:'50%',background:'#fca5a5',animation:'blink 1.2s step-start infinite',display:'inline-block'}}/>ALERT
//           </div>
//           <div style={{flex:1,overflow:'hidden'}}>
//             <div style={{display:'inline-block',whiteSpace:'nowrap',animation:'ticker 100s linear infinite',fontSize:13,fontWeight:600,color:'#fecaca',fontFamily:FONT_BODY}}>
//               &nbsp;&nbsp;&nbsp;{activeAlerts.map(a=>`⚠ ${a.city}: ${a.warning}`).join('   ·   ')}&nbsp;&nbsp;&nbsp;·&nbsp;&nbsp;&nbsp;{activeAlerts.map(a=>`⚠ ${a.city}: ${a.warning}`).join('   ·   ')}&nbsp;&nbsp;&nbsp;
//             </div>
//           </div>
//         </div>
...
//       {/* HEADER */}
//       <div style={{padding:'14px 22px',display:'flex',alignItems:'center',gap:16,borderBottom:'1px solid rgba(255,255,255,0.07)',background:'rgba(0,0,0,0.25)',backdropFilter:'blur(20px)',flexShrink:0,zIndex:10}}>
//         <div style={{display:'flex',alignItems:'center',gap:9,flexShrink:0}}>
...
//             <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
//           </div>
//           <div>
//             <div style={{fontFamily:FONT_HEAD,fontWeight:800,fontSize:16,letterSpacing:'-0.01em',lineHeight:1}}>IMD AI Hub</div>
//             <div style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.4)',marginTop:2}}>India Meteorological Department</div>
//           </div>
//         </div>
//         <SearchBar value={search} onChange={setSearch} onClear={()=>setSearch('')} accent={accent} placeholder="Search city, state or condition…"/>
//         <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
//           <LiveClock accent={accent} format={timeFormat}/>
//           <button onClick={()=>setTimeFormat(f=>f==='12h'?'24h':'12h')} className="icon-btn"
//             style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',color:'rgba(255,255,255,0.85)',borderRadius:8,padding:'7px 14px',fontSize:13,fontWeight:600,fontFamily:FONT_MONO}}>{timeFormat}</button>
//           <button onClick={()=>setUnit(u=>u==='C'?'F':'C')} className="icon-btn"
//             style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',color:'rgba(255,255,255,0.85)',borderRadius:8,padding:'7px 14px',fontSize:13,fontWeight:600,fontFamily:FONT_MONO}}>°{unit}</button>
//           <div style={{display:'flex',alignItems:'center',gap:6,background:'rgba(255,255,255,0.06)',borderRadius:8,padding:'6px 12px',fontSize:13,fontWeight:600,border:'1px solid rgba(255,255,255,0.1)'}}>
//             {isDay
//               ? <WiDaySunny size={18} color={accent}/>
//               : getMoonPhaseIcon(18,accent)
//             }
//             <span style={{color:'rgba(255,255,255,0.65)'}}>{isDay?'Day':'Night'}</span>
//           </div>
//           <div style={{display:'flex',alignItems:'center',gap:6,background:'rgba(255,255,255,0.06)',borderRadius:8,padding:'6px 12px',fontSize:13,fontWeight:700,border:'1px solid rgba(255,255,255,0.1)'}}>
//             <span style={{width:6,height:6,borderRadius:'50%',background:'#4ade80',animation:'blink 2.5s ease infinite'}}/>Live
//           </div>
//         </div>
//       </div>
...
//               {['all','state'].map(mode=>(
//                 <button key={mode} onClick={()=>setLeftView(mode)}
//                   style={{flex:1,padding:'6px 0',borderRadius:6,fontSize:12,fontWeight:600,border:'none',
//                     fontFamily:FONT_BODY,cursor:'pointer',
//                     background:leftView===mode?accent:'transparent',
//                     color:leftView===mode?'#000':'rgba(255,255,255,0.55)',
//                     textTransform:'capitalize',transition:'all 0.18s'}}>
//                   {mode==='all'?'All Cities':'By State'}
//                 </button>
//               ))}
//             </div>
//             <div style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.3)',marginTop:6,paddingLeft:2,fontFamily:FONT_MONO}}>
//               {filteredStats.length} {filteredStats.length===1?'city':'cities'}
//               {leftView==='state'&&` · ${stateGroups.length} states`}
//             </div>
//           </div>
...
//               <>
//                 {filteredStats.length===0&&(
//                   <div style={{textAlign:'center',padding:'28px 10px',color:'rgba(255,255,255,0.35)',fontSize:14,fontWeight:500}}>
//                     No cities match "{search}"
//                   </div>
//                 )}
//                 {filteredStats.map((city,i)=>{
...
//                         animation:`slideRight 0.25s ${Math.min(i,15)*0.04}s both`}}>
//                       <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
//                         <div style={{fontWeight:700,fontSize:14,fontFamily:FONT_HEAD}}>{city.city}</div>
//                         <div style={{display:'flex',alignItems:'center'}}>
//                           {getWeatherIcon(city.condition,ct.isDay,22,isActive?ct.accent:'rgba(255,255,255,0.8)')}
//                         </div>
//                       </div>
//                       <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:5}}>
//                         <div style={{fontSize:22,fontWeight:800,fontFamily:FONT_HEAD,color:isActive?ct.accent:'#fff'}}>
//                           {unit==='C'?city.temperature:toF(city.temperature)}°
//                         </div>
//                         {city.warning&&city.warning!=='None'
//                           ?<div style={{fontSize:11,fontWeight:700,background:'#dc2626',color:'#fff',borderRadius:4,padding:'2px 7px',letterSpacing:'0.04em'}}>⚠ ALERT</div>
//                           :<div style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.4)'}}>{city.humidity}% hum</div>}
//                       </div>
//                       <div style={{fontSize:12,fontWeight:500,color:'rgba(255,255,255,0.4)',marginTop:2}}>{city.condition}</div>
//                     </div>
//                   );
//                 })}
//               </>
...
//                         <div style={{flex:1}}>
//                           <div style={{display:'flex',alignItems:'center',gap:5}}>
//                             <span style={{fontWeight:700,fontSize:13,fontFamily:FONT_HEAD,color:hasActiveCity?accent:'rgba(255,255,255,0.88)'}}>
//                               {state}
//                             </span>
//                             {hasAlert&&<span style={{fontSize:10,fontWeight:700,background:'#dc2626',color:'#fff',borderRadius:3,padding:'1px 5px'}}>⚠</span>}
//                           </div>
//                           <div style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.35)',marginTop:1,fontFamily:FONT_MONO}}>
//                             {cities.length} {cities.length===1?'city':'cities'} · avg {unit==='C'?avgTemp:toF(avgTemp)}°
//                           </div>
//                         </div>
//                         <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',transform:isOpen?'rotate(90deg)':'rotate(0deg)',transition:'transform 0.18s'}}>›</div>
//                       </div>
//                       {isOpen&&cities.map((city,i)=>{
//                         const ct=getTheme(city.condition,city.sunrise,city.sunset);
//                         const isActive=selected?.city===city.city;
...
//                               borderLeft:`3px solid ${isActive?ct.accent:'rgba(255,255,255,0.08)'}`}}>
//                             <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
//                               <div style={{fontWeight:700,fontSize:13,fontFamily:FONT_HEAD}}>{city.city}</div>
//                               <div style={{display:'flex',alignItems:'center'}}>
//                                 {getWeatherIcon(city.condition,ct.isDay,18,isActive?ct.accent:'rgba(255,255,255,0.75)')}
//                               </div>
//                             </div>
//                             <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:3}}>
//                               <div style={{fontSize:18,fontWeight:800,fontFamily:FONT_HEAD,color:isActive?ct.accent:'#fff'}}>
//                                 {unit==='C'?city.temperature:toF(city.temperature)}°
//                               </div>
//                               {city.warning&&city.warning!=='None'
//                                 ?<div style={{fontSize:10,fontWeight:700,background:'#dc2626',color:'#fff',borderRadius:3,padding:'2px 6px'}}>⚠</div>
//                                 :<div style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.4)'}}>{city.humidity}%</div>}
//                             </div>
//                             <div style={{fontSize:11,fontWeight:500,color:'rgba(255,255,255,0.4)',marginTop:1}}>{city.condition}</div>
//                           </div>
//                         );
//                       })}
//                     </div>
...
//                 <div>
//                   <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
//                     <div style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.45)',display:'flex',alignItems:'center',gap:5}}>
//                       <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
//                       {selected.city}{selected.state&&`, ${selected.state}`}
//                     </div>
//                     <span style={{width:3,height:3,borderRadius:'50%',background:'rgba(255,255,255,0.25)'}}/>
//                     <div style={{fontSize:13,fontWeight:500,color:'rgba(255,255,255,0.4)'}}>Updated just now</div>
//                     <div style={{display:'flex',alignItems:'center',marginLeft:4}}>
//                       {isDay ? <WiDaySunny size={18} color={accent}/> : getMoonPhaseIcon(18,accent)}
//                     </div>
//                   </div>
//                   <div style={{display:'flex',alignItems:'flex-end',gap:8}}>
//                     <div style={{fontFamily:FONT_HEAD,fontSize:92,fontWeight:900,lineHeight:0.9,letterSpacing:'-0.03em',color:'#fff',textShadow:`0 0 60px ${glow}`}}>
//                       {unit==='C'?selected.temperature:toF(selected.temperature)}
//                     </div>
//                     <div style={{paddingBottom:14}}><div style={{fontFamily:FONT_HEAD,fontSize:34,color:'rgba(255,255,255,0.55)',fontWeight:600}}>°{unit}</div></div>
//                     <div style={{paddingBottom:8,display:'flex',alignItems:'center'}}>
//                       {getWeatherIcon(selected.condition,isDay,60,'rgba(255,255,255,0.9)')}
//                     </div>
//                   </div>
//                   <div style={{fontFamily:FONT_HEAD,fontSize:21,color:'rgba(255,255,255,0.72)',marginTop:4,fontWeight:700}}>{selected.condition}</div>
//                   <div style={{display:'flex',gap:16,marginTop:8,fontSize:14,fontWeight:500,color:'rgba(255,255,255,0.45)'}}>
//                     <span>Feels like <b style={{color:'rgba(255,255,255,0.85)',fontWeight:700}}>{displayTemp(selected.feels_like||selected.temperature-2)}</b></span>
//                     <span>H: <b style={{color:accent,fontWeight:700}}>{displayTemp(selected.high||selected.temperature+3)}</b></span>
//                     <span>L: <b style={{color:'#38bdf8',fontWeight:700}}>{displayTemp(selected.low||selected.temperature-5)}</b></span>
//                   </div>
//                 </div>
//                 <div style={{display:'flex',flexDirection:'column',gap:10,flexShrink:0}}>
//                   {selected.warning&&selected.warning!=='None'&&(
//                     <div style={{background:'rgba(220,38,38,0.12)',border:'1px solid rgba(220,38,38,0.35)',borderRadius:14,padding:'12px 16px',maxWidth:200,backdropFilter:'blur(10px)'}}>
//                       <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:6}}>
//                         {(w=>{
//                           const c=selected.warning.toLowerCase();
//                           if(c.match(/heat|hot|temperature/))   return <WiHot size={20} color="#f87171"/>;
...
//                           return <WiThunderstorm size={20} color="#fca5a5"/>;
//                         })(selected.warning)}
//                         <span style={{fontWeight:800,fontSize:12,color:'#fca5a5',letterSpacing:'0.05em',textTransform:'uppercase'}}>Active Alert</span>
//                       </div>
//                       <div style={{fontSize:13,fontWeight:500,color:'rgba(255,255,255,0.6)',lineHeight:1.5}}>{selected.warning}</div>
//                     </div>
//                   )}
//                   <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:'12px 16px',backdropFilter:'blur(10px)'}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.4)',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.06em',display:'flex',alignItems:'center',gap:6}}>
//                       <WiSunrise size={16} color={accent}/> Sunrise / <WiSunset size={16} color={accent}/> Sunset
//                     </div>
//                     <SunArc sunrise={selected.sunrise||'06:15'} sunset={selected.sunset||'18:40'} accent={accent}/>
//                   </div>
...
//                 {['overview','hourly','weekly','analysis'].map(tab=>(
//                   <button key={tab} className="tab" onClick={()=>setActiveTab(tab)}
//                     style={{flex:1,padding:'9px 0',borderRadius:9,fontSize:14,fontWeight:activeTab===tab?700:500,
//                       background:activeTab===tab?accent:'transparent',
//                       color:activeTab===tab?'#000':'rgba(255,255,255,0.55)',
//                       textTransform:'capitalize',transition:'all 0.2s',letterSpacing:'0.005em'}}>
//                     {tab}
//                   </button>
//                 ))}
//               </div>
...
//                   <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:16}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:16,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>Temperature &amp; Humidity — All Cities</div>
//                     <div style={{height:220}}>
//                       <ResponsiveContainer width="100%" height="100%">
//                         <ComposedChart data={stats} margin={{top:4,right:8,left:-10,bottom:0}}>
//                           <defs>
...
//                           </defs>
//                           <CartesianGrid strokeDasharray="2 4" stroke="rgba(255,255,255,0.05)" vertical={false}/>
//                           <XAxis dataKey="city" stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)',fontFamily:FONT_BODY}} axisLine={false} tickLine={false}/>
//                           <YAxis stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)'}} axisLine={false} tickLine={false}/>
//                           <Tooltip content={<ChartTooltip/>}/>
//                           <Bar dataKey="temperature" name="Temp °C" fill={accent} radius={[5,5,0,0]} opacity={0.8} maxBarSize={36}/>
//                           <Line dataKey="humidity" name="Humidity %" stroke="#38bdf8" strokeWidth={2} dot={{r:3,fill:'#38bdf8'}} activeDot={{r:5}}/>
//                         </ComposedChart>
...
//                   <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
//                     <div style={{background:'rgba(255,255,255,0.04)',borderRadius:14,padding:'14px 16px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                       <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.4)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10}}>{isDay?'Daytime':'Moon Phase'}</div>
//                       <div style={{marginBottom:4,display:'flex',alignItems:'center'}}>
//                         {isDay
//                           ? getWeatherIcon(selected.condition,true,40,accent)
//                           : getMoonPhaseIcon(40,accent)
//                         }
//                       </div>
//                       <div style={{fontSize:14,fontWeight:500,color:'rgba(255,255,255,0.55)'}}>{isDay?`${selected.condition} day`:"Tonight's sky"}</div>
//                     </div>
//                     <div style={{background:'rgba(255,255,255,0.04)',borderRadius:14,padding:'14px 16px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                       <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.4)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10,display:'flex',alignItems:'center',gap:6}}>
//                         <WiCloudRefresh size={16} color="rgba(255,255,255,0.4)"/> Cloud Cover
//                       </div>
//                       <div style={{fontSize:30,fontWeight:800,fontFamily:FONT_HEAD,color:'#fff',letterSpacing:'-0.01em'}}>{selected.cloud_cover||35}%</div>
//                       <div style={{marginTop:8,height:4,background:'rgba(255,255,255,0.07)',borderRadius:4}}>
//                         <div style={{height:'100%',width:`${selected.cloud_cover||35}%`,borderRadius:4,background:'rgba(148,163,184,0.6)',transition:'width 0.5s ease'}}/>
//                       </div>
//                     </div>
...
//                 <div style={{animation:'fadeIn 0.3s ease'}}>
//                   <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:16}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>24-Hour Forecast · {selected.city}</div>
//                     {extended.hourly.length>0
//                       ? <HourlyForecast data={extended.hourly} accent={accent}/>
//                       : <div style={{fontSize:13,fontWeight:500,color:'rgba(255,255,255,0.35)',padding:'10px 0'}}>No hourly data yet — waiting for the next pipeline update.</div>}
//                     {extended.hourly.length>0&&(
//                       <div style={{marginTop:20,height:160}}>
//                         <ResponsiveContainer width="100%" height="100%">
//                           <AreaChart data={extended.hourly} margin={{top:4,right:8,left:-10,bottom:0}}>
...
//                             </defs>
//                             <CartesianGrid strokeDasharray="2 4" stroke="rgba(255,255,255,0.05)" vertical={false}/>
//                             <XAxis dataKey="hour" stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)',fontFamily:FONT_MONO}} axisLine={false} tickLine={false}/>
//                             <YAxis stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)'}} axisLine={false} tickLine={false}/>
//                             <Tooltip content={<ChartTooltip/>}/>
//                             <Area type="monotone" dataKey="temp" name="Temp °C" stroke={accent} fill="url(#hg)" strokeWidth={2.5} dot={false} activeDot={{r:4,fill:accent}}/>
//                           </AreaChart>
...
//                 <div style={{animation:'fadeIn 0.3s ease'}}>
//                   <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>7-Day Forecast (IMD + Open-Meteo Hybrid) · {selected.city}</div>
//                     {extended.weekly.length>0
//                       ? <WeekForecast data={extended.weekly} accent={accent}/>
//                       : <div style={{fontSize:13,fontWeight:500,color:'rgba(255,255,255,0.35)',padding:'10px 0'}}>No 7-day forecast yet — run the updater to populate weather_forecasts.</div>}
//                   </div>
//                 </div>
//               )}
...
//                 <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,animation:'fadeIn 0.3s ease'}}>
//                   <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:10,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>Radar</div>
//                     <ResponsiveContainer width="100%" height={200}>
//                       <RadarChart data={radarData}>
//                         <PolarGrid stroke="rgba(255,255,255,0.08)"/>
//                         <PolarAngleAxis dataKey="m" tick={{fill:'rgba(255,255,255,0.5)',fontSize:12,fontWeight:600,fontFamily:FONT_BODY}}/>
//                         <Radar dataKey="v" stroke={accent} fill={accent} fillOpacity={0.2} strokeWidth={1.5}/>
//                       </RadarChart>
//                     </ResponsiveContainer>
//                   </div>
//                   <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:10,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO,display:'flex',alignItems:'center',gap:6}}>
//                       <WiWindDeg size={16} color={accent}/> Wind · {selected.city}
//                     </div>
//                     <WindRose deg={selected.wind_deg||225} speed={selected.wind_speed||12} gust={selected.wind_gust||18} accent={accent}/>
//                   </div>
//                   <div style={{gridColumn:'1/-1',background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)'}}>
//                     <div style={{fontSize:12,fontWeight:700,color:'rgba(255,255,255,0.5)',marginBottom:12,textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:FONT_MONO}}>All-City Comparison</div>
//                     <ResponsiveContainer width="100%" height={170}>
//                       <ComposedChart data={stats} margin={{top:4,right:8,left:-10,bottom:0}}>
//                         <CartesianGrid strokeDasharray="2 4" stroke="rgba(255,255,255,0.05)" vertical={false}/>
//                         <XAxis dataKey="city" stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)',fontFamily:FONT_BODY}} axisLine={false} tickLine={false}/>
//                         <YAxis stroke="rgba(255,255,255,0.2)" tick={{fontSize:12,fontWeight:600,fill:'rgba(255,255,255,0.5)'}} axisLine={false} tickLine={false}/>
//                         <Tooltip content={<ChartTooltip/>}/>
//                         <Legend wrapperStyle={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.55)',paddingTop:8}}/>
//                         <Bar dataKey="humidity" name="Humidity %" fill="#38bdf8" radius={[4,4,0,0]} opacity={0.65} maxBarSize={30}/>
//                         <Line dataKey="temperature" name="Temp °C" stroke={accent} strokeWidth={2.5} dot={{r:3,fill:accent}} activeDot={{r:5}}/>
//                       </ComposedChart>
//                     </ResponsiveContainer>
...
//     sunrise:d.sunrise||'06:15', sunset:d.sunset||'18:45',
//     state:d.state||'',
//     hourly_timeline: Array.isArray(d.hourly_timeline) ? d.hourly_timeline : [],
//   };
// }
...
const FONT_LINK = "https://fonts.googleapis.com/css2?family=Source+Serif+4:opsz,wght@8..60,400;8..60,500;8..60,600;8..60,700;8..60,800&family=Inter:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap";
...
.city-row{transition:background 0.18s,transform 0.18s;cursor:pointer;}
.city-row:hover{background:rgba(255,255,255,0.08)!important;transform:translateX(3px);}
.tab{transition:all 0.2s ease;cursor:pointer;border:none;font-family:'Inter',sans-serif;}
.tab:hover{opacity:1!important;}
.chip{transition:all 0.15s ease;cursor:pointer;}
.chip:hover{opacity:1!important;transform:translateY(-1px);}
.icon-btn{transition:all 0.2s ease;cursor:pointer;}
...
function WeatherParticles({ accent }) {
  ...
  useEffect(() => {
    // Generate the random particles only on the client side
  ...
  // Wait until mounted before rendering anything
...
function LiveClock({ accent, format='12h' }) {
  ...
  // Render an invisible box of the same size during SSR to prevent layout shifting
  if (!mounted) {
    return <div style={{ height: 38, width: 100 }}></div>;
  }
...
function WindRose({ deg=0, speed=0, accent, gust=0 }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
      ...
      <div style={{textAlign:'center'}}>
        <div style={{fontSize:26,fontWeight:700,color:'#fff',fontFamily:"'Inter',sans-serif",letterSpacing:'-0.01em'}}>{speed}<span style={{fontSize:13,color:'rgba(255,255,255,0.4)',fontWeight:400}}> km/h</span></div>
        <div style={{fontSize:11,color:accent,marginTop:2}}>{compassDir(deg)} · Gusts {gust} km/h</div>
      </div>
    </div>
  );
      ...
      {data.map((d,i)=>{
        ...
        return (
          <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',borderRadius:12,
            ...
            border:`1px solid ${i===0?`${accent}30`:'rgba(255,255,255,0.05)'}`,
            animation:`slideRight 0.3s ${i*0.06}s both`}}>
            <div style={{fontSize:13,color:'rgba(255,255,255,0.6)',width:32,fontFamily:"'Inter',sans-serif"}}>{d.day}</div>
            <div style={{fontSize:26,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center',width:32}}>
              {d.icon}
            </div>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',width:90,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.condition}</div>
  ...
  return (
    <div style={{position:'relative',flex:1,maxWidth:440}}>
      ...
      <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||"Search city…"} className="search-input"
        style={{width:'100%',background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',
          borderRadius:12,padding:'9px 36px 9px 38px',color:'#fff',fontSize:13,
          fontFamily:"'Inter',sans-serif",transition:'all 0.25s',backdropFilter:'blur(10px)'}}
        onFocus={e=>{e.target.style.border=`1px solid ${accent}66`;e.target.style.background='rgba(255,255,255,0.1)';}}
        onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.12)';e.target.style.background='rgba(255,255,255,0.07)';}}/>
      {value&&<button onClick={onClear} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',
        background:'rgba(255,255,255,0.12)',border:'none',borderRadius:'50%',width:18,height:18,
...
function StatCard({ icon, label, value, sub, subColor, progress, accentColor }) {
  return (
    <div className="stat-card" style={{background:'rgba(255,255,255,0.05)',borderRadius:14,
      padding:'16px 18px',border:'1px solid rgba(255,255,255,0.08)',overflow:'hidden',position:'relative'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
        <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',textTransform:'uppercase',letterSpacing:'0.1em',fontFamily:"'Inter',sans-serif",fontWeight:600}}>{label}</div>
        <span style={{fontSize:24,lineHeight:1,display:'flex',alignItems:'center'}}>{icon}</span>
      </div>
      <div style={{fontSize:24,fontWeight:700,color:'#fff',fontFamily:"'Inter',sans-serif",lineHeight:1,letterSpacing:'-0.01em'}}>{value}</div>
      {sub&&<div style={{fontSize:11,color:subColor||'rgba(255,255,255,0.45)',marginTop:5}}>{sub}</div>}
      {progress!=null&&(
        <div style={{marginTop:10,height:3,background:'rgba(255,255,255,0.07)',borderRadius:4}}>
          <div style={{height:'100%',width:`${Math.min(100,progress)}%`,borderRadius:4,background:accentColor||'rgba(255,255,255,0.4)',transition:'width 0.5s ease'}}/>
    ...
    <div style={{flex:1}}>
      ...
      <div style={{height:data.length*28}}>
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          <BarChart data={data} layout="vertical" margin={{top:0,right:36,left:4,bottom:0}}>
            <XAxis type="number" domain={['auto','auto']} hide/>
            <YAxis type="category" dataKey="city" width={72} tick={{fontSize:11,fill:'rgba(255,255,255,0.6)',fontFamily:"'Inter',sans-serif"}} axisLine={false} tickLine={false}/>
            <Tooltip content={<ChartTooltip/>} cursor={{fill:'rgba(255,255,255,0.04)'}}/>
            <Bar dataKey="temp" name="Temp °C" fill={barColor} radius={[0,5,5,0]} barSize={14}
              label={{position:'right',fontSize:10,fill:'rgba(255,255,255,0.55)',formatter:v=>`${v}°`}}/>
          </BarChart>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{padding:'10px 12px',borderBottom:'1px solid rgba(255,255,255,0.05)',display:'flex',gap:5,flexWrap:'wrap',flexShrink:0}}>
        {quickQs.map(q=>(
          <button key={q} className="chip" onClick={()=>setQuery(q)}
            style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.65)',borderRadius:20,padding:'4px 10px',fontSize:10,cursor:'pointer',fontFamily:"'Inter',sans-serif",opacity:0.85}}>
            {q}
          </button>
        ))}
      </div>
      ...
      <div style={{padding:'12px 14px',borderTop:'1px solid rgba(255,255,255,0.07)',flexShrink:0}}>
        <div style={{display:'flex',gap:8,background:'rgba(255,255,255,0.06)',borderRadius:13,padding:'4px 4px 4px 14px',border:`1px solid ${isListening?`${accent}60`:'rgba(255,255,255,0.1)'}`,transition:'border-color 0.2s',backdropFilter:'blur(10px)'}}>
          <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={handleKey} disabled={isTyping}
            style={{flex:1,background:'transparent',border:'none',outline:'none',color:'#fff',fontSize:13,fontFamily:"'Inter',sans-serif",opacity:isTyping?0.5:1}}
            placeholder={isListening?'Listening — speak now…':isTyping?'Thinking…':'Ask anything about weather…'}/>
          {voiceSupported&&(
            <button onClick={toggleVoice} disabled={isTyping} title={isListening?'Stop':'Voice input'}
              style={{background:isListening?'rgba(239,68,68,0.25)':'rgba(255,255,255,0.08)',border:isListening?'1px solid rgba(239,68,68,0.5)':'1px solid rgba(255,255,255,0.12)',borderRadius:8,padding:'7px 10px',cursor:isTyping?'not-allowed':'pointer',fontSize:14,opacity:isTyping?0.4:1,transition:'background 0.2s,border 0.2s',display:'flex',alignItems:'center',justifyContent:'center'}}>
          ...
          <button onClick={handleChat} disabled={isTyping||!query.trim()}
            style={{background:query.trim()?accent:'rgba(255,255,255,0.1)',border:'none',borderRadius:10,padding:'8px 16px',color:query.trim()?'#000':'rgba(255,255,255,0.3)',fontWeight:700,fontSize:13,cursor:(isTyping||!query.trim())?'not-allowed':'pointer',fontFamily:"'Inter',sans-serif",opacity:isTyping?0.45:1,transition:'all 0.2s',letterSpacing:'-0.01em'}}>
            ↑
          </button>
        </div>
        {voiceSupported&&!isListening&&<div style={{fontSize:10,color:'rgba(255,255,255,0.2)',marginTop:5,textAlign:'center'}}>🎤 tap mic to speak · Enter to send</div>}
...
export default function WeatherDashboard() {
  ...
  const [mobileView, setMobileView]     = useState('dashboard'); // 'cities', 'dashboard', 'chat'
  ...
  // Automatically switch to dashboard view on mobile when a city is selected
  const selectCity=useCallback(city=>{
    ...
    <div style={{height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden',
      ...
      {activeAlerts.length>0&&(
        <div style={{background:'linear-gradient(90deg,#7f1d1d,#991b1b)',height:34,display:'flex',alignItems:'center',overflow:'hidden',flexShrink:0,zIndex:20}}>
          ...
          <div style={{flex:1,overflow:'hidden'}}>
            <div style={{display:'inline-block',whiteSpace:'nowrap',animation:'ticker 100s linear infinite',fontSize:11,color:'#fecaca',fontFamily:"'Inter',sans-serif"}}>
              &nbsp;&nbsp;&nbsp;{activeAlerts.map(a=>`⚠ ${a.city}: ${a.warning}`).join('   ·   ')}&nbsp;&nbsp;&nbsp;·&nbsp;&nbsp;&nbsp;{activeAlerts.map(a=>`⚠ ${a.city}: ${a.warning}`).join('   ·   ')}&nbsp;&nbsp;&nbsp;
            </div>
          </div>
        </div>
      ...
      {/* HEADER */}
      <div style={{padding:'12px 22px',display:'flex',alignItems:'center',gap:16,borderBottom:'1px solid rgba(255,255,255,0.07)',background:'rgba(0,0,0,0.25)',backdropFilter:'blur(20px)',flexShrink:0,zIndex:10, flexWrap: isMobile ? 'wrap' : 'nowrap'}}>
        <div style={{display:'flex',alignItems:'center',gap:9,flexShrink:0}}>
          <div style={{width:34,height:34,borderRadius:10,background:`${accent}22`,border:`1px solid ${accent}44`,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
          </div>
          <div>
            <div style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:14,letterSpacing:'-0.01em',lineHeight:1}}>IMD AI Hub</div>
            <div style={{fontSize:10,color:'rgba(255,255,255,0.35)',marginTop:1}}>India Meteorological Dept</div>
          </div>
        </div>
        
        {/* Search wraps to full width on mobile */}
        <div style={{ flex: 1, minWidth: isMobile ? '100%' : 'auto', order: isMobile ? 3 : 2 }}>
          <SearchBar value={search} onChange={setSearch} onClear={()=>setSearch('')} accent={accent} placeholder="Search city, state or condition…"/>
        </div>

        <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,flexShrink:0, order: isMobile ? 2 : 3}}>
          {!isMobile && <LiveClock accent={accent} format={timeFormat}/>}
          <button onClick={()=>setTimeFormat(f=>f==='12h'?'24h':'12h')} className="icon-btn"
            style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',color:'rgba(255,255,255,0.8)',borderRadius:8,padding:'6px 13px',fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>{timeFormat}</button>
          <button onClick={()=>setUnit(u=>u==='C'?'F':'C')} className="icon-btn"
            style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.12)',color:'rgba(255,255,255,0.8)',borderRadius:8,padding:'6px 13px',fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>°{unit}</button>
      ...
      <div style={{display: isMobile ? 'flex' : 'grid', gridTemplateColumns: isMobile ? 'none' : '230px 1fr 310px', flexDirection: 'column', flex:1, minHeight:0}}>

        {/* ── LEFT: CITY LIST ─────────────────────────────────────────── */}
        <div style={{ display: (!isMobile || mobileView === 'cities') ? 'flex' : 'none', flexDirection:'column', overflow:'hidden', borderRight: isMobile ? 'none' : '1px solid rgba(255,255,255,0.07)', flex: isMobile ? 1 : 'unset'}}>
          <div style={{padding:'10px 10px 6px',flexShrink:0,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
            <div style={{display:'flex',gap:4,background:'rgba(255,255,255,0.05)',borderRadius:9,padding:3}}>
              {['all','state'].map(mode=>(
                <button key={mode} onClick={()=>setLeftView(mode)}
                  ...
                  return (
                    <div key={i} className="city-row" onClick={()=>selectCity(city)}
                      style={{padding:'11px 12px',borderRadius:12,marginBottom:3,
                        ...
                        animation:`slideRight 0.25s ${Math.min(i,15)*0.04}s both`}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <div style={{fontWeight:600,fontSize:13,fontFamily:"'Inter',sans-serif"}}>{city.city}</div>
                        <div style={{display:'flex',alignItems:'center'}}>
                          {getWeatherIcon(city.condition,ct.isDay,22,isActive?ct.accent:'rgba(255,255,255,0.75)')}
                        </div>
                      </div>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:5}}>
                        <div style={{fontSize:20,fontWeight:800,fontFamily:"'Inter',sans-serif",color:isActive?ct.accent:'#fff'}}>
                          {unit==='C'?city.temperature:toF(city.temperature)}°
                        </div>
                        {city.warning&&city.warning!=='None'
                          ?<div style={{fontSize:9,background:'#dc2626',color:'#fff',borderRadius:4,padding:'2px 6px',letterSpacing:'0.05em'}}>⚠ ALERT</div>
                  ...
                  return (
                    <div key={state} style={{marginBottom:5}}>
                      <div className="state-header" onClick={()=>toggleState(state)}
                        ...
                        <div style={{flex:1}}>
                          <div style={{display:'flex',alignItems:'center',gap:5}}>
                            <span style={{fontWeight:600,fontSize:12,fontFamily:"'Inter',sans-serif",color:hasActiveCity?accent:'rgba(255,255,255,0.85)'}}>
                              {state}
                            </span>
                            {hasAlert&&<span style={{fontSize:8,background:'#dc2626',color:'#fff',borderRadius:3,padding:'1px 4px'}}>⚠</span>}
                          </div>
                        ...
                        return (
                          <div key={i} className="city-row" onClick={()=>selectCity(city)}
                            style={{padding:'9px 12px 9px 18px',borderRadius:10,marginBottom:2,
                              ...
                              borderLeft:`3px solid ${isActive?ct.accent:'rgba(255,255,255,0.08)'}`}}>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                              <div style={{fontWeight:600,fontSize:12,fontFamily:"'Inter',sans-serif"}}>{city.city}</div>
                              <div style={{display:'flex',alignItems:'center'}}>
                                {getWeatherIcon(city.condition,ct.isDay,18,isActive?ct.accent:'rgba(255,255,255,0.7)')}
                              </div>
                            </div>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:3}}>
                              <div style={{fontSize:17,fontWeight:800,fontFamily:"'Inter',sans-serif",color:isActive?ct.accent:'#fff'}}>
                                {unit==='C'?city.temperature:toF(city.temperature)}°
                              </div>
                              {city.warning&&city.warning!=='None'
                                ?<div style={{fontSize:8,background:'#dc2626',color:'#fff',borderRadius:3,padding:'2px 5px'}}>⚠</div>
        ...
        {/* ── CENTER: MAIN PANEL ──────────────────────────────────────── */}
        <div style={{ display: (!isMobile || mobileView === 'dashboard') ? 'block' : 'none', overflowY:'auto', padding: isMobile ? '16px' : '22px 26px', opacity:transitioning?0:1, transition:'opacity 0.2s ease', scrollBehavior:'smooth', flex: isMobile ? 1 : 'unset'}}>
          {selected&&(
            <div style={{animation:'fadeUp 0.35s ease'}}>
              {/* Hero */}
              <div style={{display:'flex',alignItems: isMobile ? 'center' : 'flex-start',justifyContent:'space-between', flexDirection: isMobile ? 'column' : 'row', marginBottom:32, gap:16, textAlign: isMobile ? 'center' : 'left'}}>
                <div style={{ width: isMobile ? '100%' : 'auto', display: 'flex', flexDirection: 'column', alignItems: isMobile ? 'center' : 'flex-start' }}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                    <div style={{fontSize:13,color:'rgba(255,255,255,0.45)',display:'flex',alignItems:'center',gap:5,fontWeight:600,letterSpacing:'0.02em'}}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                      {selected.city}{selected.state&&`, ${selected.state}`}
                    </div>
                    <span style={{width:3,height:3,borderRadius:'50%',background:'rgba(255,255,255,0.2)'}}/>
                    <div style={{fontSize:12,color:'rgba(255,255,255,0.35)'}}>Just now</div>
                    {!isMobile && (
                      <div style={{display:'flex',alignItems:'center',marginLeft:4}}>
                        {isDay ? <WiDaySunny size={20} color={accent}/> : getMoonPhaseIcon(20,accent)}
                      </div>
                    )}
                  </div>
                  <div style={{display:'flex',alignItems:'flex-end', justifyContent: isMobile ? 'center' : 'flex-start', gap:10}}>
                    <div style={{fontFamily:"'Source Serif 4',serif",fontSize: isMobile ? 96 : 132,fontWeight:700,lineHeight:0.85,letterSpacing:'-0.02em',color:'#fff',textShadow:`0 0 70px ${glow}`}}>
                      {unit==='C'?selected.temperature:toF(selected.temperature)}
                    </div>
                    <div style={{paddingBottom: isMobile ? 12 : 22}}><div style={{fontFamily:"'Source Serif 4',serif",fontSize: isMobile ? 32 : 44,color:'rgba(255,255,255,0.5)',fontWeight:500}}>°{unit}</div></div>
                    <div style={{paddingBottom: isMobile ? 6 : 12,display:'flex',alignItems:'center'}}>
                      {getWeatherIcon(selected.condition,isDay, isMobile ? 60 : 80,'rgba(255,255,255,0.88)')}
                    </div>
                  </div>
                  <div style={{fontFamily:"'Inter',sans-serif",fontSize:26,color:'rgba(255,255,255,0.7)',marginTop:6,fontWeight:600,letterSpacing:'-0.01em'}}>{selected.condition}</div>
                  <div style={{display:'flex', gap:18, marginTop:10, fontSize:14, color:'rgba(255,255,255,0.45)'}}>
                    <span>Feels <b style={{color:'rgba(255,255,255,0.8)',fontWeight:700}}>{displayTemp(selected.feels_like||selected.temperature-2)}</b></span>
                    <span>H: <b style={{color:accent,fontWeight:700}}>{displayTemp(selected.high||selected.temperature+3)}</b></span>
                    <span>L: <b style={{color:'#38bdf8',fontWeight:700}}>{displayTemp(selected.low||selected.temperature-5)}</b></span>
                  </div>
                </div>
                <div style={{display:'flex',flexDirection: isMobile ? 'row' : 'column', width: isMobile ? '100%' : 'auto', gap:10, flexShrink:0}}>
                  {selected.warning&&selected.warning!=='None'&&(
                    <div style={{background:'rgba(220,38,38,0.12)',border:'1px solid rgba(220,38,38,0.35)',borderRadius:14,padding:'12px 16px', flex: isMobile ? 1 : 'unset', maxWidth: isMobile ? '100%' : 190, backdropFilter:'blur(10px)', textAlign: 'left'}}>
                      <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:6}}>
                        {(w=>{
                          const c=selected.warning.toLowerCase();
                          if(c.match(/heat|hot|temperature/))   return <WiHot size={20} color="#f87171"/>;
                        ...
                        <span style={{fontWeight:700,fontSize:11,color:'#fca5a5',letterSpacing:'0.06em',textTransform:'uppercase'}}>Alert</span>
                      </div>
                      <div style={{fontSize:11,color:'rgba(255,255,255,0.55)',lineHeight:1.5}}>{selected.warning}</div>
                    </div>
                  )}
                  <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:'12px 16px', flex: isMobile ? 1 : 'unset', backdropFilter:'blur(10px)'}}>
                    <div style={{fontSize:10,color:'rgba(255,255,255,0.35)',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.08em',display:'flex',alignItems:'center', justifyContent: isMobile ? 'center' : 'flex-start', gap:6}}>
                      <WiSunrise size={16} color={accent}/> Sunrise / Sunset
                    </div>
                    <SunArc sunrise={selected.sunrise||'06:15'} sunset={selected.sunset||'18:40'} accent={accent}/>
                  </div>
                </div>
              ...
              <div style={{display:'flex',gap:3,marginBottom:20,background:'rgba(255,255,255,0.05)',borderRadius:12,padding:4,border:'1px solid rgba(255,255,255,0.07)'}}>
                {['overview','hourly','weekly','analysis'].map(tab=>(
                  <button key={tab} className="tab" onClick={()=>setActiveTab(tab)}
                    style={{flex:1,padding:'8px 0',borderRadius:9,fontSize: isMobile ? 10 : 12,fontWeight:activeTab===tab?700:500,
                      background:activeTab===tab?accent:'transparent',
                      color:activeTab===tab?'#000':'rgba(255,255,255,0.5)',
                      textTransform:'capitalize',transition:'all 0.2s',letterSpacing:'0.01em'}}>
                    {tab}
              ...
              {activeTab==='overview'&&(
                <div style={{animation:'fadeIn 0.3s ease'}}>
                  ...
                  <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:16}}>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:16,textTransform:'uppercase',letterSpacing:'0.1em',fontFamily:"'JetBrains Mono',monospace"}}>Temp & Humidity Comparison</div>
                    <div style={{height:220}}>
                      <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                        <ComposedChart data={stats} margin={{top:4,right:8,left:-10,bottom:0}}>
                          <defs>
                          ...
                          <CartesianGrid strokeDasharray="2 4" stroke="rgba(255,255,255,0.05)" vertical={false}/>
                          <XAxis dataKey="city" stroke="rgba(255,255,255,0.2)" tick={{fontSize:10,fill:'rgba(255,255,255,0.4)',fontFamily:"'Inter',sans-serif"}} axisLine={false} tickLine={false}/>
                          <YAxis stroke="rgba(255,255,255,0.2)" tick={{fontSize:10,fill:'rgba(255,255,255,0.4)'}} axisLine={false} tickLine={false}/>
                          <Tooltip content={<ChartTooltip/>}/>
                          <Bar dataKey="temperature" name="Temp °C" fill={accent} radius={[5,5,0,0]} opacity={0.8} maxBarSize={36}/>
                          <Line dataKey="humidity" name="Humidity %" stroke="#38bdf8" strokeWidth={2} dot={{r:3,fill:'#38bdf8'}} activeDot={{r:5}}/>
                  ...
                  <HotColdChart stats={stats} accent={accent} isMobile={isMobile}/>
                  ...
                  <div style={{display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr',gap:10}}>
                    ...
                    <div style={{background:'rgba(255,255,255,0.04)',borderRadius:14,padding:'14px 16px',border:'1px solid rgba(255,255,255,0.07)'}}>
                      ...
                      <div style={{fontSize:28,fontWeight:800,fontFamily:"'Inter',sans-serif",color:'#fff',letterSpacing:'-0.01em'}}>{selected.cloud_cover||35}%</div>
                      <div style={{marginTop:8,height:4,background:'rgba(255,255,255,0.07)',borderRadius:4}}>
                        <div style={{height:'100%',width:`${selected.cloud_cover||35}%`,borderRadius:4,background:'rgba(148,163,184,0.6)',transition:'width 0.5s ease'}}/>
                      </div>
                    </div>
              ...
              {activeTab==='hourly'&&(
                <div style={{animation:'fadeIn 0.3s ease'}}>
                  <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:16, overflowX: isMobile ? 'auto' : 'hidden'}}>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.1em',fontFamily:"'JetBrains Mono',monospace"}}>24-Hour Forecast · {selected.city}</div>
                  ...
                  <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)', overflowX: isMobile ? 'auto' : 'hidden'}}>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.1em',fontFamily:"'JetBrains Mono',monospace"}}>Precipitation Probability</div>
                    <div style={{height:120, minWidth: isMobile ? 600 : '100%'}}>
                      <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                        <AreaChart data={extended.hourly} margin={{top:4,right:8,left:-10,bottom:0}}>
                          <defs>
                            <linearGradient id="pg" x1="0" y1="0" x2="0" y2="1">
              ...
              {/* ANALYSIS */}
              {activeTab==='analysis'&&(
                <div style={{display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr',gap:14,animation:'fadeIn 0.3s ease'}}>
                  <div style={{background:'rgba(255,255,255,0.04)',borderRadius:16,padding:'18px 20px',border:'1px solid rgba(255,255,255,0.07)'}}>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:10,textTransform:'uppercase',letterSpacing:'0.1em',fontFamily:"'JetBrains Mono',monospace"}}>Radar</div>
                    <ResponsiveContainer width="100%" height={200} minWidth={1} minHeight={1}>
                      <RadarChart data={radarData}>
                        <PolarGrid stroke="rgba(255,255,255,0.08)"/>
                        <PolarAngleAxis dataKey="m" tick={{fill:'rgba(255,255,255,0.4)',fontSize:10,fontFamily:"'Inter',sans-serif"}}/>
                        <Radar dataKey="v" stroke={accent} fill={accent} fillOpacity={0.2} strokeWidth={1.5}/>
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
        ...
        {/* ── RIGHT: AI CHAT ──────────────────────────────────────────── */}
        <div style={{ display: (!isMobile || mobileView === 'chat') ? 'flex' : 'none', borderLeft: isMobile ? 'none' : '1px solid rgba(255,255,255,0.07)', flexDirection:'column', overflow:'hidden', minHeight:0, background:'rgba(0,0,0,0.15)', flex: isMobile ? 1 : 'unset'}}>
          <ChatPanel accent={accent} selected={selected}/>
        </div>
      </div>
      ...
      {isMobile && (
        <div style={{ display:'flex', background:'rgba(0,0,0,0.85)', backdropFilter:'blur(20px)', borderTop:'1px solid rgba(255,255,255,0.1)', padding:'10px', paddingBottom:'calc(10px + env(safe-area-inset-bottom))', zIndex:50, flexShrink:0 }}>
          {[
            { id: 'cities', icon: '📍', label: 'Cities' },
            { id: 'dashboard', icon: '🌤️', label: 'Weather' },
            { id: 'chat', icon: '✨', label: 'AI Chat' }
          ].map(nav => (
            <button key={nav.id} onClick={() => setMobileView(nav.id)}
              style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'transparent', border:'none', color: mobileView === nav.id ? accent : 'rgba(255,255,255,0.4)', cursor:'pointer', transition:'color 0.2s' }}>
              <span style={{ fontSize: 20 }}>{nav.icon}</span>
              <span style={{ fontSize: 10, fontWeight: mobileView === nav.id ? 700 : 500, fontFamily:"'Inter',sans-serif" }}>{nav.label}</span>
            </button>
          ))}
        </div>
      )}
